...
Vcpkg は C 及び C++ ライブラリの管理を行います。Windows、Linux、と MacOS に対応しています。
このツールとエコシステムは常に進化を続けており、我々は全ての貢献に感謝します。

もし vcpkg を使用した事が無いか、vcpkg の使用方法を探している場合は、
[入門](#入門)から vcpkg の使用法を学べます。
...
利用可能なコマンドの簡単な説明は、vcpkg をインストールしてから、
`vcpkg help` または、特定のコマンドの説明は `vcpkg help [コマンド]` を実行してください。

* GitHub: <https://github.com/microsoft/vcpkg>、プログラムは <https://github.com/microsoft/vcpkg-tool>。
* Slack: <https://cppalliance.org/slack/> の #vcpkg チャンネル
* Discord: [\#include \<C++\>](https://www.includecpp.org) の #🌏vcpkg チャンネル
* 文書: [このリポジトリの docs ディレクトリ](docs/README.md)
